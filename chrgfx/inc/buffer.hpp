...
public:
...
#ifdef DEBUG
		std::cerr << __func__ << ": Copying existing buffer from " << std::showbase
							<< std::hex << (std::size_t) data << " to "
	...
	{
#ifdef DEBUG
		std::cerr << __func__ << ": Moving existing buffer at " << std::showbase
							<< std::hex << (std::size_t) other.m_buffer << " to "
							<< (std::size_t) this->m_buffer << ", datasize "
							<< this->m_datasize << std::endl;
#endif
	};
	 ...
	 */
	explicit buffer(
		std::istream & data, size_t const block_size = DEFAULT_BLOCK_SZ)
	{
#ifdef DEBUG
		std::cerr << __func__ << ": Reading data from input stream in "
							<< std::showbase << std::hex << (std::size_t) block_size
							<< "byte blocks" << std::endl;
#endif
	...
	~buffer()
	{
		...
		{
#ifdef DEBUG
			std::cerr << __func__ << ": Freeing buffer at " << std::showbase
								<< std::hex << (std::size_t) this->m_buffer << ", datasize "
								<< this->m_datasize << std::endl;
#endif
			free(this->m_buffer);
...
#ifdef DEBUG
		std::cerr << __func__ << ": Shrunk buffer at " << std::showbase << std::hex
	...
	size_t append(size_t const size, char const initial = 0)
	{
		...
		this->m_size += size;
#ifdef DEBUG
		std::cerr << __func__ << ": Grew buffer at " << std::showbase << std::hex
...
#ifdef DEBUG
		std::cerr << __func__ << ": Appending data from other buffer at "
							<< std::showbase << std::hex << (std::size_t) other.m_buffer
							<< ", datasize " << other.m_datasize << std::endl;
#endif
	...
	size_t append(DataT * const other, size_t size)
	{
...
#ifdef DEBUG
		std::cerr << __func__ << ": Appending data via pointer to " << std::showbase
							<< std::hex << (std::size_t) other << ", datasize " << size
							<< std::endl;
#endif
		...
		std::memcpy(
			(char *) (this->m_buffer) + this->m_datasize, (char *) other, size);
...
#ifdef DEBUG
		std::cerr << __func__ << ": Copied portion of existing buffer to"
							<< std::showbase << std::hex << (std::size_t) this->m_buffer
							<< ", size " << this->m_datasize << std::endl;
#endif
	}
	...
	void stream_in(std::istream & in, std::streamsize const block_size)
	{
		...
		try
		{
			...
			while(! in.eof())
			{
				...
				std::memcpy(
					((char *) this->m_buffer) + this->m_datasize, block_buff, bytes_read);
...
#ifdef DEBUG
			std::cerr << __func__ << ": Created buffer from stream at "
								<< std::showbase << std::hex << (std::size_t) this->m_buffer
								<< ", size " << this->m_datasize << std::endl;
#endif
			free(block_buff);
