...
namespace chrgfx
{
	...
	namespace converters
	{
										...
										std::optional<converters::cvto_col_t> const & color_conv)
		{
			/*
				Note about palettes
				Since an input png can, at most, have 256 colors and since some hardware
			...
			*/
			...
			unsigned int out_datasize {
				outpal_length * (entry_datasize * subpal_length <= subpal_datasize
														 ? subpal_datasize / 8
							 ...
							 std::optional<converters::cvfrom_col_t> const & color_conv)
		{
			...
			else
			{
				...
				outpal_length =
						(((subpal_count * from_paldef.get_subpal_length()) > 256)
								 ? 256
					...
					++this_inpal_entry)
			{
				...
				{
					bit_align_ptr += subpal_datasize - (inpal_entry_datasize *
																							from_paldef.get_subpal_length());
				}
			}
