...
u8 * toFormattedChr(chrdef const & to_def, u8 const * in_data)
{
	/*
		-for every line...
		--for every pixel...
	...
	*/
	...
	auto out_data = new u8[to_def.datasize() / 8];

	// for every line...
	for(line_iter = 0; line_iter < to_def.height(); ++line_iter)
	{
		bitpos_y = to_def.rowOffsetAt(line_iter);

		// for every pixel...
		for(pixel_iter = 0; pixel_iter < to_def.width(); ++pixel_iter)
		{
			...
			bitpos_x = to_def.pixelOffsetAt(pixel_iter);
			// for every bit plane...
			for(plane_iter = 0; plane_iter < to_def.bitdepth(); ++plane_iter)
			{
				...
				bitpos = bitpos_y + bitpos_x + to_def.planeOffsetAt(plane_iter);
...
char * toBasicChr(chrdef const & from_def, char const * in_data)
{
	...
	auto out_data = new char[from_def.width() * from_def.height()];
	...
	*/
	...
	// for every line...
	for(line_iter = 0; line_iter < from_def.height(); ++line_iter)
	{
		bitpos_y = from_def.rowOffsetAt(line_iter);

		// for every pixel in the line...
		for(pixel_iter = 0; pixel_iter < from_def.width(); ++pixel_iter)
		{
			bitpos_x = from_def.pixelOffsetAt(pixel_iter);
			curr_pixel = 0;

			// for every bit planee
			for(plane_iter = 0; plane_iter < from_def.bitdepth(); ++plane_iter)
			{
				bitpos = bitpos_y + bitpos_x + from_def.planeOffsetAt(plane_iter);
