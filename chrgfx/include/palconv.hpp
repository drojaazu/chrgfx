 ...
 */
typedef u8 *(*cvto_pal_t)(paldef const &, rgbcoldef const &, png::palette const &,
 ...
 */
u8 *cvto_pal(
		paldef const &to_paldef, rgbcoldef const &to_coldef, png::palette const &data,
		std::optional<converters::cvto_col_t> const &color_conv = std::nullopt);
...
png::palette cvfrom_pal(
		...
		std::optional<converters::cvfrom_col_t> const &color_conv = std::nullopt);
