...
chrgfx::basic_image from_png(png::image<png::index_pixel> const & png_image)
{
	...
	basic_image basic_img(width, height);
	byte_t * ptr = basic_img.pixbuf();
	...
	chrgfx::basic_palette basic_pal;
	size_t counter {0};
	for (auto const & color : png_palette)
		basic_pal[counter++] = chrgfx::basic_color(color.red, color.green, color.blue);

	basic_img.palette(basic_pal);
